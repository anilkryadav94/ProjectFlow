...
export function ProjectForm({ project, userRole, nextProjectId, filteredIds }: ProjectFormProps) {
  ...
  const form = useForm<ProjectFormValues>({
    ...
    defaultValues: {
      ...project,
      emailDate: project.emailDate ? format(new Date(project.emailDate.replace(/-/g, '\/')), "yyyy-MM-dd") : "",
      allocationDate: project.allocationDate ? format(new Date(project.allocationDate.replace(/-/g, '\/')), "yyyy-MM-dd") : "",
              ...
              <Card className="border-0 shadow-none flex flex-col flex-grow h-full">
                  ...
                  <CardContent className="flex-grow overflow-y-auto">
                    <div className="grid md:grid-cols-2 gap-6">
                      ...
                      {/* Column 2 */}
                      <div className="space-y-4">
                        <FormField control={form.control} name="emailDate" render={({ field }) => (<FormItem className="flex flex-col"><FormLabel>Email Date</FormLabel><Popover><PopoverTrigger asChild><FormControl><Button variant="outline" className={cn("font-normal", !field.value && "text-muted-foreground")} disabled={!isAdminView && !isManagerView}><CalendarIcon className="mr-2 h-4 w-4" />{field.value ? format(new Date(field.value.replace(/-/g, '\/')), "PPP") : <span>Pick a date</span>}</Button></FormControl></PopoverTrigger><PopoverContent className="w-auto p-0"><Calendar mode="single" selected={field.value ? new Date(field.value.replace(/-/g, '\/')) : undefined} onSelect={(date) => field.onChange(date ? format(date, 'yyyy-MM-dd') : '')} /></PopoverContent></Popover><FormMessage /></FormItem>)} />
